...
class MatchingService {
  ...
  static Future<List<MatchedDonor>> matchDonorsWithNGO(NGOModel ngoRequest) async {
    try {
      ...
      for (var doc in donorSnapshot.docs) {
        DonorModel donorRequest = DonorModel.fromMap(doc.data() as Map<String, dynamic>);
        
        // Skip if donor already accepted by another NGO
        if (donorRequest.acceptedByNgoId != null) continue;

        Map<String, dynamic> matchResult = calculateMatch(ngoRequest, donorRequest);
        
        if (matchResult['matchScore'] > 0) {
          // Get donor details
          DocumentSnapshot donorDetails = await _firestore
              .collection('user')
              .doc(donorRequest.donorId)
              .get();

          if (donorDetails.exists) {
            Map<String, dynamic> userData = donorDetails.data() as Map<String, dynamic>;
            
            matchedDonors.add(MatchedDonor(
  ...
  static Future<List<MatchedNGO>> matchNGOsWithDonor(DonorModel donorRequest) async {
    try {
      ...
      for (var doc in ngoSnapshot.docs) {
        NGOModel ngoRequest = NGOModel.fromMap(doc.data() as Map<String, dynamic>);
        ...
        if (matchResult['matchScore'] > 0) {
          // Get NGO details
          DocumentSnapshot ngoDetails = await _firestore
              .collection('user')
              .doc(ngoRequest.ngoId)
              .get();

          if (ngoDetails.exists) {
            Map<String, dynamic> userData = ngoDetails.data() as Map<String, dynamic>;
            
            matchedNGOs.add(MatchedNGO(
  ...
  static Map<String, dynamic> calculateMatch(NGOModel ngoRequest, DonorModel donorRequest) {
    ...
    for (var item in donorRequest.items) {
      donorItems[item['name'].toString().toUpperCase()] = item['quantity'] as int;
    }
    ...
    for (String ngoItemName in ngoItems.keys) {
      ...
      if (donorItems.containsKey(ngoItemName)) {
        ...
        int fulfilledQuantity = ngoQuantity < donorQuantity ? ngoQuantity : donorQuantity;
    ...
    double overallMatchScore = totalRequestedItems > 0 ? totalMatchScore / totalRequestedItems : 0.0;
    
    // Calculate completeness bonus: donations that have more of the requested items get a slight bonus
    double completenessRatio = totalRequestedItems > 0 ? matchedItemCount / totalRequestedItems : 0.0;
  ...
  static Future<bool> acceptDonation(String ngoId, String donorRequestId) async {
    try {
      ...
      return await _firestore.runTransaction<bool>((transaction) async {
        ...
        if (donorSnapshot.docs.isEmpty) {
          print('Donor request not found or already fulfilled for requestId: $donorRequestId');
          return false;
        }
        ...
        if (donorData['acceptedByNgoId'] != null) {
          print('Donation already accepted by another NGO: ${donorData['acceptedByNgoId']}');
          return false;
        }
        ...
        transaction.update(_firestore.collection('donorfood').doc(donorDocumentId), {
        ...
        if (ngoSnapshot.docs.isNotEmpty) {
          ...
          transaction.update(_firestore.collection('ngofood').doc(ngoDocumentId), {
        ...
        print('Successfully updated donor request $donorRequestId with acceptedByNgoId: $ngoId');
        return true;
      });
    } catch (e) {
  ...
  static Future<bool> fulfillDonation(String donorRequestId, String ngoId) async {
    try {
      ...
      if (donorSnapshot.docs.isEmpty) {
        print('Donor request not found, not accepted by this NGO, or already fulfilled for requestId: $donorRequestId');
        return false;
      }
    ...
    try {
      ...
      for (var doc in donorSnapshot.docs) {
        ...
        if (donorRequest.acceptedByNgoId != null) {
          filteredCount++;
          print('Filtering out donation ${donorRequest.requestId} - already accepted by ${donorRequest.acceptedByNgoId}');
          continue;
        }
      ...
      print('getAvailableDonations: Found ${donations.length} available donations (filtered out $filteredCount accepted donations)');
  ...
  static Future<List<MatchedDonationRequest>> getMatchedDonations(NGOModel ngoRequest) async {
    try {
      ...
      for (var donation in availableDonations) {
        ...
        Map<String, dynamic> matchResult = calculateMatch(ngoRequest, donorModel);
        
        if (matchResult['matchScore'] > 0) {
          matchedDonations.add(MatchedDonationRequest(
            donation: donation,
            matchedItems: matchResult['matchedItems'],
            matchScore: matchResult['matchScore'],
          ));
  ...
  static Future<void> notifyNGOsOfNewDonation(DonorModel newDonation) async {
    try {
      print('Starting to notify NGOs of new donation: ${newDonation.requestId}');
      ...
      for (var doc in ngoSnapshot.docs) {
        ...
        Map<String, dynamic> matchResult = calculateMatch(ngoRequest, newDonation);
        
        if (matchResult['matchScore'] > 0) {
          print('Match found with NGO ${ngoRequest.ngoId}, score: ${matchResult['matchScore']}');
          
          // Update NGO request with new matched donor
          List<String> updatedMatchedDonors = List<String>.from(ngoRequest.matchedDonorIds);
          if (!updatedMatchedDonors.contains(newDonation.donorId)) {
            ...
            await _sendMatchNotificationToNGO(ngoRequest.ngoId, newDonation, matchResult['matchScore']);
          }
        }
      }
      ...
      if (matchedNgoIds.isNotEmpty) {
        print('Updating donor document with ${matchedNgoIds.length} matched NGO IDs');
        ...
        if (donorSnapshot.docs.isNotEmpty) {
          ...
          print('Successfully updated donor document with ${matchedNgoIds.length} matched NGO IDs');
        } else {
  ...
  static Future<void> _sendMatchNotificationToNGO(String ngoId, DonorModel donation, double matchScore) async {
    try {
      final notificationData = {
        "body": "New donation available with ${(matchScore * 100).toStringAsFixed(1)}% match!",
        "data": {
          "donorId": donation.donorId,
          "requestId": donation.requestId,
          "type": "new_match",
  ...
  static Future<List<AcceptedDonation>> getAcceptedDonations(String ngoId) async {
    try {
      ...
      for (var doc in donorSnapshot.docs) {
        ...
        if (donorDetails.exists) {
          ...
          acceptedDonations.add(AcceptedDonation(
            documentId: doc.id,
            donorId: donorRequest.donorId,
            requestId: donorRequest.requestId,
            items: donorRequest.items,
            acceptedDate: docData['acceptedDate'] as Timestamp? ?? Timestamp.now(),
      ...
      acceptedDonations.sort((a, b) => b.acceptedDate.compareTo(a.acceptedDate));
  ...
  static Future<List<AcceptedDonation>> getFulfilledDonations(String ngoId) async {
    try {
      ...
      for (var doc in donorSnapshot.docs) {
        ...
        if (donorDetails.exists) {
          ...
          fulfilledDonations.add(AcceptedDonation(
            documentId: doc.id,
            donorId: donorRequest.donorId,
            requestId: donorRequest.requestId,
            items: donorRequest.items,
      ...
      // Sort by accepted date (newest first)
      fulfilledDonations.sort((a, b) => b.acceptedDate.compareTo(a.acceptedDate));
  ...
  static Future<List<AcceptedDonation>> getDonorAcceptedDonations(String donorId) async {
    try {
  ...
  static Future<List<AcceptedDonation>> getDonorFulfilledDonations(String donorId) async {
    try {
  ...
  static Future<List<NGORequestHistory>> getNGORequestHistory(String ngoId) async {
    try {
      ...
      for (var doc in ngoSnapshot.docs) {
        ...
        if (ngoRequest.acceptedDonorId != null) {
          ...
          if (donorSnapshot.docs.isNotEmpty) {
            DonorModel donorRequest = DonorModel.fromMap(donorSnapshot.docs.first.data() as Map<String, dynamic>);
        ...
        requestHistory.add(NGORequestHistory(
          documentId: doc.id,
          requestId: ngoRequest.requestId,
  ...
  static Future<List<NGORequestHistory>> getNGOAcceptedRequests(String ngoId) async {
    try {
      ...
      for (var doc in ngoSnapshot.docs) {
        ...
        activeRequests.add(NGORequestHistory(
          documentId: doc.id,
          requestId: ngoRequest.requestId,
          items: ngoRequest.items,
          addedDate: ngoRequest.addeddate,
  ...
  static Future<List<NGORequestHistory>> getNGOFulfilledRequests(String ngoId) async {
    try {
      ...
      for (var doc in ngoSnapshot.docs) {
        ...
        if (isFulfilled) {
          ...
          fulfilledRequests.add(NGORequestHistory(
            documentId: doc.id,
            requestId: ngoRequest.requestId,
            items: ngoRequest.items,
            addedDate: ngoRequest.addeddate,
