...
class DataMigrationService {
  ...
  static Future<void> migrateDonorData() async {
    try {
      ...
      QuerySnapshot donorSnapshot = await _firestore.collection('donorfood').get();
      int migratedCount = 0;
      int skippedCount = 0;
      ...
      for (var doc in donorSnapshot.docs) {
        ...
        if (items.isNotEmpty) {
          ...
          await _firestore.collection('donorfood').doc(doc.id).update(newDonorModel.toMap());
          migratedCount++;
          print('Migrated donor document ${doc.id}');
        }
      }

      print('Donor migration completed: $migratedCount migrated, $skippedCount skipped');
    } catch (e) {
    ...
    try {
      ...
      for (var doc in ngoSnapshot.docs) {
        ...
        if (items.isNotEmpty) {
          ...
          // Update the document with new structure
          await _firestore.collection('ngofood').doc(doc.id).update(newNGOModel.toMap());
          migratedCount++;
          print('Migrated NGO document ${doc.id}');
        }
      }

      print('NGO migration completed: $migratedCount migrated, $skippedCount skipped');
    } catch (e) {
  ...
  static List<Map<String, dynamic>> _parseOldFormat(String oldFormat) {
    ...
    List<String> itemStrings = oldFormat.split(',').map((s) => s.trim()).toList();
  ...
  static Future<Map<String, dynamic>> getMigrationStatus() async {
    try {
      ...
      return {
        ...
        'isComplete': (donorTotal == donorMigrated) && (ngoTotal == ngoMigrated),
      };
    } catch (e) {
